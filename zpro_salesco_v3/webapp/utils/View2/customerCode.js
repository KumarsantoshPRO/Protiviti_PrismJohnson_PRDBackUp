    ...
    return {
      ...
      onCustomerCodeHelp: function (that) {
        if (!that.CustomerCodeFrag) {
          that.CustomerCodeFrag = sap.ui.xmlfragment(
            "zpj.pro.sk.sd.salescoordinator.zprosalesco.view.fragments.View2.F4s.customerCodF4",
            that
          );
          that.getView().addDependent(that.CustomerCodeFrag);
          that._CustomerCodeTemp = sap.ui
            .getCore()
            .byId("idSLCustomerCodeValueHelp")
            .clone();
          that._oTempCustCode = sap.ui
            .getCore()
            .byId("idSLCustomerCodeValueHelp")
            .clone();
        }
        var aFilter = [];
        var oFilterDomname = new Filter(
          [new Filter("Domname", FilterOperator.EQ, "KNA1")],
          false
        );
        var oFilterDomname1 = new Filter(
          [new Filter("Domname1", FilterOperator.EQ, "LARS")],
          false
        );
      ...
      onValueHelpSearch_custCode: function (oEvent, that) {
        ...
        var oFilterDomname1 = new Filter(
          [new Filter("Domname1", FilterOperator.EQ, sValue)],
          false
        );
        aFilter.push(oFilterDomname1);
        aFilter.push(oFilterDomname);
      ...
      onValueHelpConfirm_custCode: function (oEvent, that) {
        ...
        that
          ...
          .setProperty("/Name", oSelectedItem.getProperty("description"));
      },
      ...
      onCustomerCodeInputSubmit: function (oEvent, that) {
        var sTerm = oEvent.getParameter("value"),
          aFilters = [],
          oFilterDomname = new Filter(
            [new Filter("Domname", FilterOperator.EQ, "KNA1")],
            false
          ),
          oFilterDomname1 = new Filter(
            [new Filter("Domname1", FilterOperator.EQ, sTerm)],
            false
          ),
          sValue1 = "/Kunnr",
          sValue2 = "/Name",
            ...
            .read(sPath, {
              ...
              success: function (Data) {
                if (Data.results.length > 0) {
                  var JSONModelForSuggest_custCode = new JSONModel(
                    Data.results
                  );
                  that
                    .getView()
                    .setModel(
                      JSONModelForSuggest_custCode,
                      "JSONModelForSuggest_custCode"
                    );
                  that
                    .getView()
                    .getModel("JSONModelForSuggest_custCode")
                    .refresh(true);
                }
                that.getView().setBusy(false);
              }.bind(that),
      ...
      onCustomerCodeInputSuggestionSelect: function (oEvent, that) {
        var sSelectedCode;
        if (oEvent.getParameter("selectedRow").getAggregation("cells")) {
          sSelectedCode = oEvent
                ...
                if (Data.results.length === 1) {
                  if (sValue1.includes("Mvgr2")) {
                    ...
                    if (that.getView().getModel("JSONModelPayload")) {
                      ...
                      that
                        .getView()
                        .byId("idV2InpCustCode")
                        .setValue(Data.results[0].DomvalueL);
                    }
                  }
                  if (sValue2) {
                    if (that.getView().getModel("JSONModelPayload")) {
